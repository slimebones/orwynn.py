...
from orwynn.yon.server import Bus, BusCfg, StaticCodeid
...
async def test_two_err():
    ...
    r = await con.client_recv()
    assert r["codeid"] == Bus().get_cached_codeid_by_code(ecode.Val).unwrap()
    assert all(flag)
    con_task.cancel()
